    ...
    <div class="detail-content">
      ...
      <div class="detail-meta">
        ...
        <div class="dm-actions">
          <button class="tb-btn" on:click={() => onCommitAction('cherry-pick', commit?.hash ?? '')}>Cherry-pick</button>
          <button class="tb-btn" on:click={() => onCommitAction('revert',      commit?.hash ?? '')}>Revert</button>
          <button class="tb-btn" on:click={() => onCommitAction('copy',        commit?.hash ?? '')}>Copy hash</button>
        </div>
      </div>
      ...
      <div class="detail-diff">
        ...
        {:else if hunks.length === 0}
          <div class="diff-empty">No diff</div>
        {:else}
          {#each hunks as h}
...
<style>
  ...
  .diff-empty { padding: 16px; color: var(--vscode-disabledForeground, #333); font-style: italic; font-size: var(--hg-font-sm); }
</style>
