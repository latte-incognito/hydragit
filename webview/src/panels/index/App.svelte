  ...
  async function branchAction(a: string) {
    ...
    const acts: Record<string, () => Promise<void>> = {
      checkout: async () => {
        ...
        flash(`Checked out ${ctxBranch}`, '#4ec94e');
        loadAll();
      },
      merge: async () => {
        await send('merge', { branch: ctxBranch });
        flash(`Merged ${ctxBranch}`, '#4ec94e');
        loadAll();
      },
      rebase: async () => {
        await send('rebase', { onto: ctxBranch });
        flash('Rebased', '#4ec94e');
        loadAll();
      },
      push: async () => {
        await send('push', { branch: ctxBranch });
        flash(`Pushed ${ctxBranch}`, '#4ec94e');
        loadAll();
      },
      delete: async () => {
      ...
      copy: async () => {
        flash(`Copied: ${ctxBranch}`, '#4ec94e');
      },
      rename: async () => {
        const to = prompt('New name:');
        if (to) {
          await send('branch.rename', { from: ctxBranch, to });
      ...
      'new-from': async () => {
        ...
        if (name) {
          await send('branch.create', { name, from: ctxBranch });
